...
where isyeri in (select marka_isim from markalar
  ...
  şehirlerini listeleyiniz. 
    ...
    -- parantez içindeki kod output olmuyor, bize yardım ediyor
    -- üst satırdaki kod bir tablodan, alt satırdaki kod diğer tablodan veri çekiyor, karışmaması için bu şekilde yazılıyor 
    /* -----------------------------------------------------------------------------
...
select marka_isim, calisan_sayisi, (select SUM(maas) from calisanlar
 ...
 select marka_isim, calisan_sayisi, ( select round(avg (maas)) from calisanlar
										WHERE marka_isim=isyeri) ortalama_maas 
 from markalar;
 /* ----------------------------------------------------------------------------
